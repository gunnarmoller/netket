...
#ifndef NETKET_VARIATIONALMONTECARLODM_HPP
#define NETKET_VARIATIONALMONTECARLODM_HPP
...
class VariationalMonteCarloDM {
  ...
  std::vector<Complex> obs_values_;
 ...
 public:
  ...
  void Init(int nsamples, int discarded_samples, int discarded_samples_on_init,
    ...
    if (target_ != "energy" && target_ != "variance" && target_ != "cost") {
      InvalidInputError(
          "Target minimization should be either energy or variance or cost\n");
    }
  ...
  void ComputeObservablesDM() 
  {
    for (std::size_t i = 0; i < obs_.size(); ++i) 
    {
      auto ex = vmc::ExpectationDM(vmc_data_, psi_, *obs_[i]);
      obs_values_[i] = ex;
    }
  }  
  ...
  void Advance(Index steps = 1) {
    ...
    for (Index i = 0; i < steps; ++i) {
      ...
      } else if (target_ == "cost") {
        grad_ = vmc::GradientDM(vmc_data_, psi_, ham_, gamma_);  
      } else {
